...
function TransactionsTab({ data, update, accountNamesIn, accountNamesOut }) {
  // ── 폼 상태
  const EMPTY_FORM = {
    id: "", date: todayISO(), type: "지출",
    cat1: "", cat2: "", amount: "",
    inAccount: "", outAccount: "", content: "", memo: "",
  };
  ...
  const filterCat1Opts = useMemo(() => {
    if (!filterType) {
      return [...new Set(
        Object.values(data.categories).flatMap(g => Object.keys(g))
      )].sort();
    }
  ...
  const filtered = useMemo(() => {
    ...
    return [...data.transactions]
      .filter(t => {
        ...
        if (q) {
          const hay = [t.content, t.memo, t.cat1, t.cat2, t.inAccount, t.outAccount]
            .join(" ").toLowerCase();
  ...
  const activeFilterCount = [
    filterMonth !== thisMonthISO(),
    !!filterType, !!filterCat1, !!search.trim(),
  ].filter(Boolean).length;

  const resetFilters = () => {
    setSearch(""); setFilterMonth(thisMonthISO()); setFilterType(""); setFilterCat1("");
  };

  // ── 저장
  const save = () => {
    if (!form.date || !form.type || !form.cat1 || !form.cat2 || n(form.amount) <= 0 || !form.content)
      return alert("필수값을 확인하세요.");
    if (form.type === "수입"    && !form.inAccount)
      return alert("수입은 입금계좌가 필요합니다.");
    if (form.type === "지출"    && !form.outAccount)
      return alert("지출은 출금계좌가 필요합니다.");
    if (form.type === "자산이동" && (!form.inAccount || !form.outAccount))
      return alert("자산이동은 입출금 계좌가 모두 필요합니다.");
    update(d => {
      const row  = { ...form, amount: n(form.amount), id: form.id || uid() };
      const list = form.id
        ? d.transactions.map(t => t.id === form.id ? row : t)
        : [...d.transactions, row];
  ...
  const Hl = ({ text = "" }) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark style={{ background: "rgba(108,125,255,.28)", color: "var(--accent2)", borderRadius: 3, padding: "0 1px" }}>
      ...
      <div className="card">
        <div className="card-title">
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text3)", fontSize: 12, padding: "2px 6px" }}>
            {showForm ? "▲ 접기" : "▼ 펼치기"}
          </button>
        </div>
        {showForm && (
          <>
            <div className="form-grid">
              ...
              <Field label="금액"><input value={form.amount} placeholder="0" onChange={e => setForm({ ...form, amount: e.target.value })} /></Field>
              <Field label="입금계좌">
              ...
              <Field label="내용"><input value={form.content} placeholder="내용 입력" onChange={e => setForm({ ...form, content: e.target.value })} /></Field>
            </div>
            ...
            <div className="form-actions">
              ...
              <button className="btn btn-ghost" onClick={() => setForm(EMPTY_FORM)}>초기화</button>
            </div>
          </>
        )}
      </div>
      ...
      <div className="card">
        <div className="card-title">
          <h3>
            ...
            <span style={{ fontSize: 12, fontWeight: 400, color: "var(--text3)", marginLeft: 8 }}>
              전체 {data.transactions.length}건 중 {filtered.length}건
            </span>
          </h3>
          {activeFilterCount > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 99, background: "var(--accent-bg)", color: "var(--accent)", fontSize: 11, fontWeight: 600 }}>
                필터 {activeFilterCount}개 적용
              </span>
              <button onClick={resetFilters} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 99, border: "none", cursor: "pointer", background: "var(--surface3)", color: "var(--text2)", fontSize: 11, fontWeight: 600 }}>
                ✕ 초기화
              </button>
            </div>
          )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          <div style={{ position: "relative" }}>
            ...
            />
            {search && (
              <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--text3)", fontSize: 14, lineHeight: 1 }}>✕</button>
            )}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 10, marginBottom: 14, alignItems: "center" }}>
          ...
          <select style={inpS} value={filterCat1} onChange={e => setFilterCat1(e.target.value)}>
            ...
            {filterCat1Opts.map(x => <option key={x}>{x}</option>)}
          </select>
          {activeFilterCount > 0
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "var(--surface2)", borderRadius: 10, marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                ...
                <span style={{ color: "var(--text3)" }}>수입</span>
                <span style={{ color: "var(--green)", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>+{fmt(subtotal.income)}</span>
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--red)", display: "inline-block" }} />
                <span style={{ color: "var(--text3)" }}>지출</span>
                <span style={{ color: "var(--red)", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>−{fmt(subtotal.expense)}</span>
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--accent)", display: "inline-block" }} />
                <span style={{ color: "var(--text3)" }}>순수입</span>
                <span style={{ color: subtotal.net >= 0 ? "var(--green)" : "var(--red)", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>
                  {subtotal.net >= 0 ? "+" : "−"}{fmt(Math.abs(subtotal.net))}
                </span>
              </span>
            </div>
            ...
            <tbody>
              {filtered.map(t => (
                <tr key={t.id}>
                  <td>{t.date}</td>
                  <td>
                    <span className={`badge ${t.type === "수입" ? "badge-green" : t.type === "지출" ? "badge-red" : "badge-muted"}`}>
                      {t.type}
                    </span>
                  </td>
                  <td className="td-name">{t.cat1}</td>
                  <td>{t.cat2}</td>
                  <td className="td-right td-mono" style={{ color: t.type === "수입" ? "var(--green)" : t.type === "지출" ? "var(--red)" : "inherit" }}>
                    {fmt(t.amount)}
                  </td>
                  <td>{t.inAccount}</td>
                  <td>{t.outAccount}</td>
              ...
              {!filtered.length && (
                <tr><td colSpan={9}><div className="empty">
                  {data.transactions.length === 0 ? "거래내역이 없습니다." : "검색 결과가 없습니다. 필터를 조정해보세요."}
                </div></td></tr>
              )}
            </tbody>
          </table>
        ...
        {filtered.length > 0 && (
          <div style={{ marginTop: 10, fontSize: 11, color: "var(--text3)", textAlign: "right" }}>
            {filterMonth || "전체 기간"} · {filtered.length}건 표시
          </div>
        )}
      </div>
    </div>
